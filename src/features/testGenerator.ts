...
export type TestingFramework = 'jest' | 'vitest' | 'mocha' | 'pytest' | 'junit';

export class TestGenerator {
    constructor(private providerManager: ProviderManager, private codeParser: CodeParser) {}
     ...
     */
    public async generateUnitTests(document: vscode.TextDocument, range: vscode.Range, framework: TestingFramework): Promise<string | null> {
        const codeToTest = document.getText(range);
        if (codeToTest.trim().length === 0) return null;
        ...
        try {
            ...
            logger.error(`Error generating unit tests: ${error.message}`);
     ...
     */
    public async scaffoldIntegrationTest(document: vscode.TextDocument, framework: TestingFramework): Promise<string | null> {
        ...
        try {
            ...
            logger.error(`Error scaffolding integration test: ${error.message}`);
     ...
     */
    public async suggestCoverageCases(document: vscode.TextDocument, range: vscode.Range): Promise<string | null> {
        const codeToAnalyze = document.getText(range);
        if (codeToAnalyze.trim().length === 0) return null;
        ...
        try {
            ...
            logger.error(`Error suggesting coverage cases: ${error.message}`);
