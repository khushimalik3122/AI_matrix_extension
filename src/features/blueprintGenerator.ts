...
import { ProjectBlueprint } from '../types/promptProcessing';
...
export class BlueprintGenerator {
     ...
     */
    public async generate(blueprint: ProjectBlueprint): Promise<DetailedBlueprint | null> {
        ...
        try {
            ...
            logger.log(`Successfully generated blueprint with ${detailedBlueprint.fileStructure.length} files and ${detailedBlueprint.dependencies.length} dependencies.`);
            return detailedBlueprint as DetailedBlueprint;

        } catch (error: any) {
            logger.error(`Error generating detailed blueprint: ${error.message}`);
            return null;
        }
    }

    private constructPrompt(blueprint: ProjectBlueprint): string {
        return `
    ...
    private parseJsonResponse(jsonString: string): any {
        try {
            ...
            logger.error(`Failed to parse JSON response for blueprint: ${jsonString}`);
