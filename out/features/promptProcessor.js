...
class PromptProcessor {
    ...
    async processRequest(userInput) {
        ...
        try {
            ...
            const blueprint = parsedJson;
            this.conversationHistory.push(`AI: Planning project '${blueprint.projectName}'.`);
            return blueprint;
        }
        catch (error) {
            logger_1.logger.error(`Error processing project request: ${error.message}`);
    ...
    parseJsonResponse(jsonString) {
        try {
            ...
            logger_1.logger.error(`Failed to parse JSON response: ${jsonString}`);
...
//# sourceMappingURL=promptProcessor.js.map